...
class Heap{
    ...
    public void heapify(int arr[][], int arrLength, int node) { 
        ...
        if(leftNode<arrLength && arr[leftNode][0] > arr[largestRoot][0]) { 
            largestRoot = leftNode; 
        } else if (leftNode<arrLength && arr[leftNode][0] == arr[largestRoot][0] && arr[leftNode][1] > arr[largestRoot][1] ) {
            largestRoot = leftNode; 
        }
        
        if(rightNode<arrLength && arr[rightNode][0] > arr[largestRoot][0]) { 
            largestRoot = rightNode;  
        } else if (rightNode<arrLength && arr[rightNode][0] == arr[largestRoot][0] && arr[rightNode][1] > arr[largestRoot][1]) {
            largestRoot = rightNode;
        }
